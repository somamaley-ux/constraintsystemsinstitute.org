...
<div class="paper-actions"><a class="text-link" href="../../pdf/${paper.id}.pdf">Read the full paper (PDF)</a><a class="text-link" href="../../pdf/${paper.id}.pdf" download>Download PDF</a></div>
...
const catalogue = [...new Set(records.map(p => p.group))].map(group => `<section class="paper-group"><h3>${escape(group)}</h3>${records.filter(p => p.group===group).map(p => `<article class="paper-entry"><span class="paper-code">${escape(p.code)}</span><div><h4><a href="papers/${p.id}/index.html">${escape(p.title)}</a></h4><p>${escape(p.description)}</p></div><a href="pdf/${p.id}.pdf" aria-label="Read ${escape(p.code)} PDF" title="Read ${escape(p.code)} PDF"><i data-lucide="file-text" aria-hidden="true"></i></a></article>`).join('\n')}</section>`).join('\n');
...
if(home.includes(homeStart) && home.includes(homeEnd)) {
  const links = records.map(p=>`<a href="standard-model/papers/${p.id}/index.html" data-paper-link data-paper-description="${escape(p.description)}" data-paper-title="${escape(p.title)}">${escape(p.code)}: ${escape(p.title)}</a>`).join('\n');
...
for(const p of records) shared.push({title:p.title,url:`${origin}/standard-model/papers/${p.id}/`,pdf_url:`${origin}/standard-model/pdf/${p.id}.pdf`,...(p.date?{date:p.date}:{}),context:`Standard Model / ${p.group}`,description:p.description,technical:p.technical,author:'Amos Jay Maley',keywords:`Standard Model, AASC, ${p.code}, ${p.group}`});
