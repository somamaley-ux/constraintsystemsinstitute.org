...
The neutrino tour is maintained directly in `neutrinos/`; its pure teaching models are checked with `node tools/check-neutrino-model.cjs`. Run the shared preparation step after edits. The compact card and sharing artwork are built by `node tools/build-neutrino-assets.cjs` with Sharp available.
